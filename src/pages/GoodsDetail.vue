...
<template>
  <div>
    <back-btn />
    <div v-if="res"
         style="width:100vw;height: 50vh;position:absolute;top:0;background-image:url(/others/bg.jpg);z-index: -1"></div>
      ...
      <div class="row justify-between">
        ...
        <div>数量
          <q-icon style="border: 1px solid black;border-radius: 100%" class="q-mx-sm bg-black text-white" @click="num++"
                  name="add" />
          {{ num }}
          <q-icon style="border: 1px solid black;border-radius: 100%" class="q-mx-sm bg-black text-white"
                  @click="num=num===0?num+1:1"
                  name="remove" />
        </div>
      </div>
      <div class="q-ma-md text-grey-10">{{ res.detail }}</div>
      <div class="row justify-center q-mt-xl">
        <q-btn style="border-radius: 4rem;bottom:4.5rem" @click="handleClick"
               class="fixed q-px-xl bg-grey-5 text-white text-h6">立 即 购 买
        </q-btn>
      </div>
    </div>
    <div v-else>
